...
export default function EventHighlights() {
  const highlightCards = highlights.map((item, i) => {
    const Icon = item.icon
    return (
      <div
        key={i}
        className="p-8 rounded-lg border border-border bg-card/50 hover:bg-card hover:border-accent/50 transition-all duration-300 animate-fade-in group h-full"
  ...
  return (
    <section className="py-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6">{highlightCards}</div>

        <div className="md:hidden">
          <Carousel items={highlightCards} itemsPerView={1} />
        </div>
      </div>
    </section>
  )
